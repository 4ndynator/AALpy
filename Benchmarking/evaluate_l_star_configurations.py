...
for size in automata_size:
    for i in input_sizes:
        for o in output_sizes:
            random_model = generate_random_deterministic_automata('dfa', size, i, o, num_accepting_states=150)
            test_models.append(random_model)

tc = 0
num_exp = len(test_models) * len(closing_strategies) * len(closedness_types) * num_repeats * len(cex_processing)
        ...
        for closing_strategy in closing_strategies:
            for cex in cex_processing:
                for _ in range(num_repeats):
                    ...
                    config_name = f'E_set_type_{closedness_type}_closing_strategy_{closing_strategy}_cex_{cex}'
